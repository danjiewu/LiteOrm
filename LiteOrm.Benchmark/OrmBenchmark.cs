...
namespace LiteOrm.Benchmark
{
    ...
    public class OrmBenchmark
    {
        ...
        private const string ConnectionString = "Server=localhost;User ID=ormbench;Password=orm!123;Database=OrmBench;AllowLoadLocalInfile=true;";
        ...
        public void Setup()
        {
            try
            {
                _host = Host.CreateDefaultBuilder()
                    .ConfigureAppConfiguration((hostingContext, config) =>
                    {
                        config.AddInMemoryCollection(new Dictionary<string, string?>
                        {
                            ...
                            ["LiteOrm:ConnectionStrings:0:ConnectionString"] = ConnectionString,
                            ["LiteOrm:ConnectionStrings:0:Provider"] = "MySqlConnector.MySqlConnection, MySqlConnector",
                    ...
                    .ConfigureServices(services =>
                    {
                        ...
                        // 3. FreeSql 配置
                        services.AddSingleton<IFreeSql>(s =>
                        {
                ...
                using (var scope = _serviceProvider.CreateScope())
                {
                    ...
                    var efUsers = Enumerable.Range(1, QueryCount).Select(i => new BenchmarkUser { Name = $"User{i}", Age = 20 + (i % 50), Email = $"user{i}@example.com", CreateTime = DateTime.Now }).ToList();
                    ...
                    var sugarUsers = Enumerable.Range(1, QueryCount).Select(i => new BenchmarkUser { Name = $"User{i}", Age = 20 + (i % 50), Email = $"user{i}@example.com", CreateTime = DateTime.Now }).ToList();
                    ...
                    var fsqlUsers = Enumerable.Range(1, QueryCount).Select(i => new BenchmarkUser { Name = $"User{i}", Age = 20 + (i % 50), Email = $"user{i}@example.com", CreateTime = DateTime.Now }).ToList();
                    ...
                    var liteUsers = Enumerable.Range(1, QueryCount).Select(i => new BenchmarkUser { Name = $"User{i}", Age = 20 + (i % 50), Email = $"user{i}@example.com", CreateTime = DateTime.Now }).ToList();
        ...
        public async Task EFCore_Insert_Async()
        {
            using (var scope = _serviceProvider.CreateScope())
            {
                ...
                var users = Enumerable.Range(1, InsertCount).Select(i => new BenchmarkUser { Name = "EF", Age = 25, Email = "ef@test.com", CreateTime = DateTime.Now }).ToList();
        ...
        public async Task SqlSugar_Insert_Async()
        {
            ...
            {
                var sugar = scope.ServiceProvider.GetRequiredService<ISqlSugarClient>();
                var users = Enumerable.Range(1, InsertCount).Select(i => new BenchmarkUser { Name = "Sugar", Age = 25, Email = "sugar@test.com", CreateTime = DateTime.Now }).ToList();
        ...
        public async Task LiteOrm_Insert_Async()
        {
            using (var scope = _serviceProvider.CreateScope())
            {
                ...
                var users = Enumerable.Range(1, InsertCount).Select(i => new BenchmarkUser { Name = "Lite", Age = 25, Email = "lite@test.com", CreateTime = DateTime.Now }).ToList();
        ...
        public async Task Dapper_Insert_Async()
        {
            ...
            {
                var users = Enumerable.Range(1, InsertCount).Select(i => new BenchmarkUser { Name = "Dapper", Age = 25, Email = "dapper@test.com", CreateTime = DateTime.Now }).ToList();
        ...
        public async Task FreeSql_Insert_Async()
        {
            ...
            {
                var fsql = scope.ServiceProvider.GetRequiredService<IFreeSql>();
                var users = Enumerable.Range(1, InsertCount).Select(i => new BenchmarkUser { Name = "FreeSql", Age = 25, Email = "freesql@test.com", CreateTime = DateTime.Now }).ToList();
        ...
        public async Task EFCore_Update_Async()
        {
            ...
            {
                var db = scope.ServiceProvider.GetRequiredService<BenchmarkDbContext>();
                var users = await db.BenchmarkUsers.Take(InsertCount).ToListAsync();
        ...
        public async Task SqlSugar_Update_Async()
        {
            ...
            {
                var sugar = scope.ServiceProvider.GetRequiredService<ISqlSugarClient>();
                var users = await sugar.Queryable<BenchmarkUser>().Take(InsertCount).ToListAsync();
                foreach (var u in users)
                {
        ...
        public async Task LiteOrm_Update_Async()
        {
            using (var scope = _serviceProvider.CreateScope())
            {
                ...
                var users = await viewService.SearchSectionAsync(null, new PageSection(0, InsertCount));
                foreach (var u in users)
                {
        ...
        public async Task Dapper_Update_Async()
        {
            using (var conn = new MySqlConnection(ConnectionString))
            {
                var users = (await conn.QueryAsync<BenchmarkUser>($"SELECT * FROM BenchmarkUser LIMIT {InsertCount}")).ToList();
                foreach (var u in users)
                {
        ...
        public async Task FreeSql_Update_Async()
        {
            ...
            {
                var fsql = scope.ServiceProvider.GetRequiredService<IFreeSql>();
                var users = await fsql.Select<BenchmarkUser>().Limit(InsertCount).ToListAsync();
                foreach (var u in users)
                {
        ...
        public async Task EFCore_UpdateOrInsert_Async()
        {
            ...
            {
                var db = scope.ServiceProvider.GetRequiredService<BenchmarkDbContext>();
                var existingUsers = await db.BenchmarkUsers.Take(InsertCount / 2).ToListAsync();
                ...
                var newUsers = Enumerable.Range(1, InsertCount / 2).Select(i => new BenchmarkUser 
                { 
                    Name = "EF_Upsert_I", 
        ...
        public async Task SqlSugar_UpdateOrInsert_Async()
        {
            ...
            {
                var sugar = scope.ServiceProvider.GetRequiredService<ISqlSugarClient>();
                var existingUsers = await sugar.Queryable<BenchmarkUser>().Take(InsertCount / 2).ToListAsync();
                foreach (var u in existingUsers) { u.Name = "Sugar_Upsert_U"; u.Age = _random.Next(20, 60); }
                var newUsers = Enumerable.Range(1, InsertCount / 2).Select(i => new BenchmarkUser { Name = "Sugar_Upsert_I", Age = _random.Next(20, 60), Email = $"sugar_upsert{i}@test.com", CreateTime = DateTime.Now }).ToList();
        ...
        public async Task LiteOrm_UpdateOrInsert_Async()
        {
            using (var scope = _serviceProvider.CreateScope())
            {
                var viewService = scope.ServiceProvider.GetRequiredService<IEntityViewServiceAsync<BenchmarkUser>>();
                var service = scope.ServiceProvider.GetRequiredService<IEntityServiceAsync<BenchmarkUser>>();
                var existingUsers = await viewService.SearchSectionAsync(null, new PageSection(0, InsertCount / 2));
                foreach (var u in existingUsers) { u.Name = "Lite_Upsert_U"; u.Age = _random.Next(20, 60); }
                var newUsers = Enumerable.Range(1, InsertCount / 2).Select(i => new BenchmarkUser { Name = "Lite_Upsert_I", Age = _random.Next(20, 60), Email = $"lite_upsert{i}@test.com", CreateTime = DateTime.Now }).ToList();
                var all = existingUsers.Concat(newUsers).ToList();
        ...
        public async Task Dapper_UpdateOrInsert_Async()
        {
            using (var conn = new MySqlConnection(ConnectionString))
            {
                var sql = "SELECT * FROM BenchmarkUser LIMIT " + (InsertCount / 2);
                ...
                var newUsers = Enumerable.Range(1, InsertCount / 2).Select(i => new BenchmarkUser { Id = 0, Name = "Dapper_Upsert_I", Age = _random.Next(20, 60), Email = $"dapper_upsert{i}@test.com", CreateTime = DateTime.Now }).ToList();
                var all = existingUsers.Concat(newUsers).ToList();
                var upsertSql = @"
        ...
        public async Task FreeSql_UpdateOrInsert_Async()
        {
            ...
            {
                var fsql = scope.ServiceProvider.GetRequiredService<IFreeSql>();
                var existingUsers = await fsql.Select<BenchmarkUser>().Limit(InsertCount / 2).ToListAsync();
                foreach (var u in existingUsers) { u.Name = "FreeSql_Upsert_U"; u.Age = _random.Next(20, 60); }
                var newUsers = Enumerable.Range(1, InsertCount / 2).Select(i => new BenchmarkUser { Name = "FreeSql_Upsert_I", Age = _random.Next(20, 60), Email = $"freesql_upsert{i}@test.com", CreateTime = DateTime.Now }).ToList();
                var all = existingUsers.Concat(newUsers).ToList();
