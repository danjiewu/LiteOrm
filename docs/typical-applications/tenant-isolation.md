...
- 批量更新、批量删除同样要带范围条件，写法见[数据权限典型应用](./data-permission.md)的场景 2。
...
- 参数值通过 `outputParams.Add` 走参数化，不会拼进 SQL 文本。安全边界见[安全性](../advanced-topics/security.md)。
- 构件返回 `null` 或空串时整个片段不产生 SQL，查询条件里不需要写 `"1 = 1"` 这类恒真条件来凑语法。写入路径没有这个保证，见[数据权限典型应用](./data-permission.md)。
...
- 关联表的切片进的是关联查询的 `JOIN ... ON`。DAO 按主键读取（`GetObject`、`ExistsKey`）走的是模型自带的 `From` 片段，这段片段只拼关联键、不带关联表的切片条件，需要连关联表一起限定时改用 `Search(...)` 这类表达式查询。完整边界见[权限过滤与用户范围控制](../di/permission-filtering.md)。
...
- [并发控制与读写分离典型应用](./concurrency-and-read-write-splitting.md)
