...
本篇聚焦各机制的**原理与用法**；面向业务的落地场景清单见[数据权限典型应用](../07-typical-applications/02-data-permission.md)。
...
安全注意事项见[安全性](../03-advanced-topics/06-security.md)中的 `GenericSqlExpr` 章节。
...
之所以这条路径能生效，是因为 DAO 和 `ExprString` 都会通过 `CreateSqlBuildContext(...)` 创建 SQL 构建上下文；当你重写它并写入 `TableArgs` 时，后续 SQL 生成自然会沿用这组路由参数。更多分表细节见[分表分库](../03-advanced-topics/01-sharding-and-tableargs.md)。
