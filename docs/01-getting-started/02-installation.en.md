...
> For older database versions where default pagination syntax is incompatible, refer to [Custom Paging](../03-advanced-topics/04-custom-paging.en.md) and [Custom SqlBuilder / Dialect Extension](../04-extensibility/03-custom-sqlbuilder.en.md).
...
- You can use it with no DI container at all via `LiteOrmContext` (see [First Full Example (Manual, No DI)](./04-first-example-manual.en.md)), or go through plain MS DI with `AddLiteOrm()` (see [First Full Example (Base Library Only)](./03-first-example.en.md)).
