...
`AutoRegisterAttribute` 从 `LiteOrm.Common` 迁移到 `LiteOrm.DependencyInjection`，命名空间随之改变：
...
可以。只要项目引用了 `LiteOrm.DependencyInjection` 并添加 `using LiteOrm.DependencyInjection;`，自动注册行为不变。
