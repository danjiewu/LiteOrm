...
| 确定性加密 | 是 | 是（用密文比较） | 否 | 需要按值精确检索的字段 |
| 盲索引列（哈希） | 否 | 是 | 否 | 只看是否匹配，不需读回原文 |
| 脱敏（掩码） | 否 | 否 | 否 | 展示用字段 |
...
**需求**：业务要支持“按证件号查客户”，但库里存的是密文。

**做法**：先接受一个事实，由 `Expr` 生成的普通 `WHERE` 参数不带列上下文。参数以 `new Param(name, value)` 的形式产生，绑定命令时只按“值的运行时类型 + 目标 `DbValueType`”查全局转换器注册表，不会去查某一列上声明的 `ConverterType`。所以下面这条查询查不到任何数据：
...
**方案一（推荐）：显式传密文，或者用盲索引列。** 加密逻辑是自己写的，转一次再比较即可，前提是加密必须是确定性的（同一明文恒定得到同一密文）：
...
场景 1 里的随机 IV 加密不满足这个前提，它每次加密结果都不同，只适合“写进去、读回来”。这类字段要做等值查询，只能加盲索引列：
...
**需求**：登录、验重这类场景只需要知道“库里有没有这个值”。

**做法**：存单向哈希，查询时哈希一次再比较：

```csharp
[Column("IdHash", DbType = DbValueType.String, Length = 64, IsIndex = true)]
public string? IdHash { get; set; }   // 值 = HMAC-SHA256(规范化明文)
```
...
- 哈希不可逆，这条列永远读不回原文，展示需要原文时另存密文列或走脱敏列。
- 哈希同样需要固定密钥（HMAC），否则明文可被彩虹表反查。
- 规范化要先做：大小写、空格、全角半角的处理方式必须固定，否则同一个值写入与查询会得到不同哈希。

## 场景 4：列表展示脱敏

**需求**：客服页面要显示手机号，但不能显示完整号码，也不希望把掩码误写回库。

**做法**：把掩码放在只读列上。列声明成 `ColumnMode.Read` 后，该列不参与 `INSERT` / `UPDATE`，读取时套掩码转换器：
...
- 日志、导出、审计都要走同一套掩码规则，只在前端做脱敏会漏掉导出文件，见[审计与变更追踪典型应用](./04-audit-and-change-tracking.md)。

## 场景 5：列长度、索引与密钥

**需求**：上线前确认列够不够长、索引建在哪、密钥怎么管。
...
**需求**：项目要用 NativeAOT 发布，确认加密列还能正常工作。
...
- 元数据来源在 AOT 下会切换成源生成的 `ColumnInfo`，见[多租户隔离典型应用](./01-tenant-isolation.md)的场景 4，加密列本身不受影响，受影响的只有 `Constant` 切片，替代做法是把固定条件写成运行时构件。
