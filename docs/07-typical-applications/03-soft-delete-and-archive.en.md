...
- Under `TableInfo` source generation (NativeAOT) the attribute slice does not produce a `ConstFilter`; turn the fixed condition into a runtime fragment as shown in scenario 4 of [Tenant Isolation in Practice](./01-tenant-isolation.en.md).
...
- Detail, update and delete still need their own checks; see scenario 3 of [Data Permissions in Practice](./02-data-permission.en.md).
