...
Writing the current user into `ConstFilter` makes every caller see the same user's data. For fixed slices see [Tenant Isolation in Practice](./01-tenant-isolation.en.md).
...
- The check must read from the master. A read replica returns lagging data and misjudges ownership; see [Concurrency and Read/Write Splitting in Practice](./06-concurrency-and-read-write-splitting.en.md).
