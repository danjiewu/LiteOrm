...
// 检查关联数据存在性
var users = await userService.SearchAsync(
    u => Exists<Department>(d => d.Id == u.DeptId)
);
...
var users = await viewService.SearchAsync(
    Lambda<TestUser>(u => Exists<TestDepartment>(d => d.Id == u.DeptId))
);

// 查询属于指定部门的用户
var usersWithSpecificDept = await viewService.SearchAsync(
    Lambda<TestUser>(u => Exists<TestDepartment>(d => d.Id == u.DeptId && d.Name == "ExistsTestDept"))
);
```
...
var notResearch = await userService.SearchAsync(
    ExistsRelated<DepartmentView>(Prop("Name").StartsWith("研")).Not()
);

// 与普通字段条件组合
var marketAdults = await userService.SearchAsync(
    ExistsRelated<DepartmentView>(Prop("Name") == "市场部")
    & (Prop("Age") > 25)
);
```
...
var deptSubQuery = From<Department>()
    .Where(Prop("Name") == "IT")
    .Select("Id");

var users = await userService.SearchAsync(
    Prop("DeptId").In(deptSubQuery)
);

// 2. EXISTS 子查询：查询至少有一个关联部门记录的用户
var usersWithDept = await userService.SearchAsync(
    Exists<Department>(Prop("Id") == Prop("T0", "DeptId"))
);
```
...
```csharp
using static LiteOrm.Common.Expr;
var result = userViewDAO.Search(Prop("Age") >= 18);
...
```csharp
using static LiteOrm.Common.Expr;
var result = dataViewDAO.Search(Prop("Age") >= 18);
...
```csharp
using static LiteOrm.Common.Expr;
var inList = await viewService.SearchAsync(Prop("Name").In("Alice", "Bob"));
var betweenList = await viewService.SearchAsync(Prop("Age").Between(30, 35));
var likeList = await viewService.SearchAsync(Prop("Name").Like("Cha%"));

var combinedList = await viewService.SearchAsync(
    Prop("Age").Between(20, 40) & Prop("Name").Contains("i")
);
```
...
var query = cteDef.With("ActiveUsers")
    .Where(Prop("Name").Contains("admin"));
```
