...
In implementation, LiteOrm merges ForeignType and TableJoin information during the metadata phase to generate JoinedTable / ForeignTable structures. For fixed-filter metadata and SQL injection details, see [Permission Filtering and User Scope Control](../06-framework/02-permission-filtering.en.md).
...
- Q: When should I use `Column.Constant`?
  A: Use it when the model itself always represents one fixed slice. For the full boundary, `ConstFilter` pipeline, and multi-tenant guidance, see [Permission Filtering and User Scope Control](../06-framework/02-permission-filtering.en.md).
