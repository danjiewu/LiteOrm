...
// 写入时使�?IArged.TableArgs => Logs_202603
await logService.InsertAsync(log);
...
�?`UserId = 25` 时，`TableArgs[0] = "UserShard_1"`，最�?SQL 标识符会变成�?
...
2. 各分片中的表结构相同，只是前缀库名�?Schema 名不同�?
...
1. 主表一旦指定了 `tableArgs`、`WithArgs(...)` �?`Expr.From<T>(...)` 的参数，这组参数会进入当�?SQL 作用域�?
...
{
    var tableName = $"{month:D2}";  // 01, 02, ... 12（表�?Logs_ 前缀已在 Table 特性中定义�?
...
| 来源                          | 优先�?| 说明                |
| --------------------------- | --- | ----------------- |
| `IArged.TableArgs`          | 自动  | 实体实现接口，插�?更新时自动使�?|
