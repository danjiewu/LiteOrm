...
If you want to use it for business scenarios such as "current-user scope filtering" or "multi-tenant filtering", read this together with [Permission Filtering](../06-framework/02-permission-filtering.en.md), which focuses on **when to use runtime Expr / GenericSqlExpr versus `ConstFilter` or table routing**.
...
Security filtering should be used in conjunction with [Permission Filtering](../06-framework/02-permission-filtering.en.md):
