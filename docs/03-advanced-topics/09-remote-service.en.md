...
### The Value of Physical Separation Between Frontend and Backend
...
### 2. Register Service and Map Endpoint
...
// Register LiteOrm main framework (local services such as EntityService)
builder.Host.RegisterLiteOrm();

// Register remote server
builder.Services.AddRemoteServer(options =>
{
    options.InvokePath = "api/remote/invoke"; // Default, must match the client
    // Optional: specify service/model namespaces to improve resolution speed
    // options.ServiceTypeResolver = new DefaultServiceTypeResolver("MyApp.Services", "MyApp.Models");
});

var app = builder.Build();

// Map the remote invocation HTTP endpoint
...
`DefaultServiceTypeResolver` constructor accepts two optional parameters: **service interface namespace** and **entity model namespace** (note: does not accept an `Assembly` parameter):

```csharp
// Default: scan all assemblies by short type name
options.ServiceTypeResolver = new DefaultServiceTypeResolver();

// Specify service interface and entity model namespaces for faster exact matching and to avoid name conflicts
options.ServiceTypeResolver = new DefaultServiceTypeResolver(
...
> **Generic type matching**: `DefaultServiceTypeResolver` uses the CLR generic name format `Foo`1` (baseName + "`" + arity) when looking up open generic types, to avoid conflicts with non-generic types of the same name.

### `TypeResolverHelper` — Bidirectional Type Name ↔ Type Resolution

`DefaultServiceTypeResolver` uses `LiteOrm.Common.TypeResolverHelper` internally to resolve `ServiceName` (short type name) to `Type`. This is a public utility class in the `LiteOrm.Common` namespace, providing bidirectional conversion between type names and `Type`. **Both the client generating `ServiceName` and the server resolving `ServiceName` depend on it.**
...
#### Use Case: Custom Service Names

By default, `ServiceName` uses the short type name. To customize it (e.g. for multi-version coexistence or name simplification), register via `TypeResolverHelper.Register`:

```csharp
using LiteOrm.Common;

// Map IUserService to "Users" (must be registered on both client and server)
TypeResolverHelper.Register("Users", typeof(IUserService));

// Afterwards, the client generates requests with ServiceName = "Users"
// The server resolves "Users" → typeof(IUserService)
```

> **Important**: `Register` must be called on **both the client and server** to ensure `ServiceName` consistency. After registration, `GetName` returns the custom name and `FindType` returns the registered type first.

### `[Service]` Attribute

The server by default scans interfaces marked with `[Service]` and `IsService == true` for registration. Unmarked interfaces are not exposed as remote services.

```csharp
[Service]                                        // Expose as remote service
...
var host = Host.CreateDefaultBuilder(args)
    .RegisterLiteOrmRemote(opts =>
    {
        ...
        opts.RemoteServicePath = "api/remote/invoke"; // Default, must match the server
        opts.AutoRegisterEntityServices = true;       // Auto-register entity service proxies
    })
    .Build();
```
...
| `AutoRegisterEntityServices` | `bool` | Whether to auto-register all entity services as remote proxies, default `false` |
...
### Auto-Register Entity Services (`AutoRegisterEntityServices`)

When set to `true`, two-step registration is performed:

**Step 1**: Register 4 open generic interface proxy implementations via MS DI `AddScoped`:
...
**Step 2**: Scan assemblies and register custom interfaces inheriting from the above generic interfaces (e.g. `IDemoUserService`) as remote proxies.

Once enabled, any entity service interface can be resolved directly from the DI container:
...
// The calling style is identical to local services
var user = await userService.GetObjectAsync(1);
```

### Manual Single Service Registration
...
```csharp
// Standalone: register one by one without AutoRegisterEntityServices
services.AddRemoteService<IUserService>()
        .AddRemoteService<IOrderService>();

// Or coexist with AutoRegisterEntityServices: manually registered interfaces are not overwritten by auto-scan
opts.AutoRegisterEntityServices = true;
services.AddRemoteService<ISpecialService>();   // Manual registration, higher priority than scan
```

| Registration Method | Applicable Scenario | Supports Non-Entity Service Interfaces |
|---------------------|---------------------|--------------------------------------|
| `AutoRegisterEntityServices` | Entity CRUD services (inheriting `IEntityService<T>`, etc.) | No (only scans derivatives of the 4 generic interfaces) |
| `AddRemoteService<TService>()` | Any service interface (including non-entity services) | Yes |
| `AddRemoteServiceGenerator<TFactory>()` | Aggregating multiple services through a factory | Yes (auto-scans factory return types) |
...
// Register factory proxy, auto-scan and register all return types as remote proxies
...
`AddRemoteServiceGenerator` scans all property and method return types of the factory interface, auto-registering types that satisfy the following conditions as remote proxies:
1. Is an interface type;
2. Namespace does not belong to `System`;
3. Not already registered in the DI container (to avoid overwriting manual registrations).
...
> **Dependency**: `IdentityOutAttribute` resolves the Identity column through `TableInfoProvider.Default`. Both client and server must register `TableInfoProvider.Default` (`LiteOrm` main library's `LiteOrmCoreInitializer` initializes it automatically).
...
## Serialization Mechanism (Core Constraints)
...
|------------|-------------|
| **Loss of reference semantics** | Parameter objects are deserialized new instances on the server; modifications to them are not automatically reflected back to the client. Use `[ArgumentOut]` attributes when write-back is needed (see below) |
...
| **`Expr` parameters serialized by declared type** | Service interface query method signatures directly accept `Expr` (e.g. `SearchAsync(Expr expr = null, ...)`). Lambda expressions written in business code (e.g. `u => u.Age > 18`) are **first converted to `Expr` derived classes** (e.g. `LogicExpr`) in the client process by extension methods (`LambdaExprConverter.ToLogicExpr`) in `LiteOrm.Common.Service.LambdaExprExtensions`, then serialized and transmitted as `Expr` type parameters. The server deserializes and reconstructs the expression tree by declared type. `Expression<Func<T,bool>>` itself is never serialized |
...
4. **`ServiceName` consistency**: The client and server use the same short type name to generate `ServiceName`, ensuring that service interface types can be correctly matched on both sides
