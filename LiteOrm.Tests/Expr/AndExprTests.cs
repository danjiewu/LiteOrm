...
namespace LiteOrm.Common.UnitTests
{
    public class AndExprTests
    {
        ...
        public void Constructor_WithIEnumerable_AddsAllItems()
        {
            ...
            var expr = new AndExpr(items.AsEnumerable());
        ...
        [Fact]
        public void Add_NestedAndExpr_FlattensItems()
        {
            var inner = new AndExpr(Condition("A", 1), Condition("B", 2));
            var outer = new AndExpr(Condition("C", 3));
        ...
        [Fact]
        public void Add_NestedOrExpr_DoesNotFlatten()
        {
            var inner = new OrExpr(Condition("A", 1), Condition("B", 2));
            var outer = new AndExpr();
        ...
        [Fact]
        public void AddRange_WithNestedAndExpr_FlattensAll()
        {
            var inner = new AndExpr(Condition("X", 1), Condition("Y", 2));
            var expr = new AndExpr(Condition("A", 1));

            expr.AddRange(new LogicExpr[] { Condition("B", 2), inner });
        ...
        [Fact]
        public void ToString_WithItems_JoinsWithAnd()
        {
            var expr = new AndExpr(Condition("A", 1), Condition("B", 2));
        ...
        [Fact]
        public void ToString_EmptyAndExpr_ReturnsEmptyString()
        {
            var expr = new AndExpr();
        ...
        {
            var left = new AndExpr(Condition("A", 1), Condition("B", 2));
            var right = new AndExpr(Condition("B", 2), Condition("A", 1));
        ...
        {
            var left = new AndExpr(Condition("A", 1), Condition("B", 2));
            var right = new AndExpr(Condition("A", 1), Condition("C", 3));
        ...
        {
            var left = new AndExpr(Condition("A", 1));
        ...
        public void Equals_DifferentDuplicateDistribution_ReturnsTrue()
        {
            ...
            var left = new AndExpr(a, a, b);
            var right = new AndExpr(a, b, b);
        ...
        [Fact]
        public void Equals_EmptyAndExpr_ReturnsTrue()
        {
            var left = new AndExpr();
            var right = new AndExpr();

            Assert.True(left.Equals(right));
        }

        [Fact]
        public void Equals_EmptyAndExpr_Vs_EmptyOrExpr_ReturnsFalse()
        {
            var left = new AndExpr();
        ...
        [Fact]
        public void GetHashCode_EqualAndExpr_ReturnsSameHash()
        {
            var left = new AndExpr(Condition("A", 1), Condition("B", 2));
            var right = new AndExpr(Condition("A", 1), Condition("B", 2));
        ...
        [Fact]
        public void GetHashCode_EmptyAndExpr_ReturnsStableHash()
        {
            var expr = new AndExpr();
        ...
        [Fact]
        public void Clone_EmptyAndExpr_CreatesNewEmptyInstance()
        {
            var expr = new AndExpr();
            var clone = (AndExpr)expr.Clone();
        ...
        {
            var item = Condition("A", 1);
            var expr = new AndExpr(item, item);
