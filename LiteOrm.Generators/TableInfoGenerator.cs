...
namespace LiteOrm.Generators
{
    ...
    public class TableInfoGenerator : IIncrementalGenerator
    {
        ...
        internal sealed class ColumnInfo
        {
            ...
            public bool IsForeign { get; set; } // 是否为 [ForeignColumn] 投影列（非真实表列）
        ...
        private static EntityInfo? BuildEntityInfo(INamedTypeSymbol type, Compilation compilation, ResolvedSymbols symbols)
        {
            ...
            foreach (var prop in GetAllInstanceProperties(type))
            {
                if (symbols.ForeignColumnAttribute != null &&
                    prop.GetAttributes().FirstOrDefault(a => SymbolEqualityComparer.Default.Equals(a.AttributeClass, symbols.ForeignColumnAttribute)) is { } foreignAttr)
                {
                    info.Columns.Add(BuildForeignColumnInfo(prop, foreignAttr, symbols));
                    continue;
                }
        ...
        private static ColumnInfo BuildForeignColumnInfo(IPropertySymbol prop, AttributeData foreignAttr, ResolvedSymbols symbols)
        {
        ...
        // ──────────────────────────────────────────────────────────────
        private static string GenerateDataReaderMappers(List<EntityInfo> entities, ResolvedSymbols symbols, Compilation compilation)
        {
            var sb = new StringBuilder();
            sb.AppendLine("// <auto-generated/>");
            sb.AppendLine("#nullable enable");
        ...
        private static void GenerateMapperMethod(StringBuilder sb, EntityInfo e, ResolvedSymbols symbols, Compilation compilation)
        {
            ...
            for (int i = 0; i < e.Columns.Count; i++)
            {
                ...
                else if (IsJsonNode(c.Symbol.Type, compilation))
                    // JsonNode 类型列：使用专门的读取方法
                    fallback = $"(System.Text.Json.Nodes.JsonNode.Parse(reader.GetString({i})))";
                else
        ...
        /// <summary>
        /// 判断列是否为JsonNode类型列：属性类型为 System.Text.Json.Nodes.JsonNode 或其派生类。
        /// </summary>
        private static bool IsJsonNode(ITypeSymbol typeSymbol, Compilation compilation)
        {
            var jsonNodeSymbol = compilation.GetTypeByMetadataName("System.Text.Json.Nodes.JsonNode");
            if (jsonNodeSymbol is null)
            {
                // 处理 JsonNode 类型在当前编译中不存在的情况
                return false;
            }

            // 使用 SymbolEqualityComparer 进行准确比较
            return SymbolEqualityComparer.Default.Equals(typeSymbol, jsonNodeSymbol);
        }

        /// <summary>
        /// 判断列是否为数组等复杂类型列：集合/数组（非 byte[]，byte[] 视为 Binary）或 DbType 为
        /// Object/Array 的列。这类列默认不生成静态列信息，需预注册转换器处理。
        /// </summary>
        ...
        private static void GenerateAotTypeRegistration(SourceProductionContext spc, Compilation compilation)
        {
            ...
            sb.AppendLine("    {");
            sb.AppendLine("        [ModuleInitializer]");
            sb.AppendLine("        [UnconditionalSuppressMessage(\"Trimming\", \"IL2111\", Justification = \"Only registers type-name mappings with TypeResolverHelper; does not invoke annotated SqlBuilder / DbConnection methods via reflection.\")]");
