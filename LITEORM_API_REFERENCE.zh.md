...
- `LiteOrm.Common/` - 核心元数据、Expr 表达式系统、接口定义
- `LiteOrm/` - ORM 核心实现、DAO 基类、SQL 构建器
- `LiteOrm.Demo/` - 示例项目
- `LiteOrm.Tests/` - 单元测试
...
#### 4.5.4 DataViewDAO - 视图查询（返回 DataTableResult）
...
- 文档： `docs/EXP/EXP_Performance.md` 中的 "BulkProvider（高性能批量提供器）" 小节。
- 示例实现： `LiteOrm.Demo.Demos.MySqlBulkCopyProvider` 展示了基于 MySqlBulkCopy 的实现。
