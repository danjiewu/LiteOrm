...
namespace LiteOrm.Demo.Demos
{
    ...
    public static class DataViewDemo
    {
        public static async Task RunAsync(IServiceProvider serviceProvider)
        {
            ...
            var resultAll = userDataView.Search(null);
            DataTable dtAll = await resultAll.GetResultAsync();
            PrintDataTable(dtAll);

            // 3. 带条件的查询 (使用 Expr)
            Console.WriteLine("\n[2] 执行条件查询 (年龄 >= 20)...");
            Expr condition = Expr.Prop("Age") >= 20;
            var resultFiltered = userDataView.Search(condition);
            DataTable dtFiltered = await resultFiltered.GetResultAsync();
            PrintDataTable(dtFiltered);

            // 4. 指定字段查询 (SelectItem)
            Console.WriteLine("\n[3] 指定字段查询 (只查 Id, UserName)...");
            string[] fields = { "Id", "UserName" };
            var resultFields = userDataView.Search(fields, condition);
            DataTable dtFields = await resultFields.GetResultAsync();
            ...
            DataTable dtComplex = await resultComplex.GetResultAsync();
